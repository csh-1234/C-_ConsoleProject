...
public:
    ...
    void DrawOutLine(int startX, int startY, int endX, int endY, const std::wstring& title = L"") {
        ...
        DrawAt(startX + 4, startY + 2, L"ÀÌ¸§ :" + wstr);
        DrawAt(startX + 4, startY + 3, L"Ã¼·Â / ÃÖ´ëÃ¼·Â : " + to_wstring(player->GetHp()) + L" / " + to_wstring(player->GetMaxHp()));
        DrawAt(startX + 4, startY + 4, L"¸¶³ª / ÃÖ´ë¸¶³ª : " + to_wstring(player->GetMp()) + L" / " + to_wstring(player->GetMaxMp()));
        DrawAt(startX + 4, startY + 5, L"°ø°Ý·Â : " + to_wstring(player->GetAtk()));
        DrawAt(startX + 4, startY + 6, L"¹æ¾î·Â : " + to_wstring(player->GetDef()));
        DrawAt(startX + 4, startY + 7, L"Å©¸®Æ¼ÄÃ È®·ü : " + to_wstring(player->GetCriRate()) + L"%");
        DrawAt(startX + 4, startY + 8, L"Å©¸®Æ¼ÄÃ µ¥¹ÌÁö :" + to_wstring(player->GetCriDamage()) + L"%");
        DrawAt(startX + 4, startY + 9, L"È¸ÇÇÀ² :" + to_wstring(player->GetAvoidRate()) + L"%");
        DrawAt(startX + 4, startY + 10, L"¼Óµµ :" + to_wstring(player->GetSpeed()));
        DrawAt(startX + 4, startY + 11, L"¼ÒÁö±Ý :" + to_wstring(player->GetMoney()) + L"°ñµå");
        ...
        DrawAt(startX + 4, startY + 12, L"ÇöÀç À§Ä¡ :" + wstr2);
    }
...
//class ConsoleBuffer {
//private:
//    std::vector<std::vector<char>> buffer;  // ANSI ¹®ÀÚ¿­ »ç¿ë
//    int width, height;
//    HANDLE hConsole;
//
//public:
...
//    void Clear() {
//        for (auto& row : buffer) {
//            std::fill(row.begin(), row.end(), ' ');  // ANSI °ø¹é »ç¿ë
//        }
//    }
//
//    void DrawAt(int x, int y, const std::string& str) {  // ANSI ¹®ÀÚ¿­ »ç¿ë
...
//    }
//
//    void ChangeAt(int x, int y, const std::string& str) {  // ANSI ¹®ÀÚ¿­ »ç¿ë
//        for (size_t i = 0; i < str.length() && x + i < width; ++i) {
//            if (y >= 0 && y < height && x + i >= 0) {
//                buffer[y][x + i] = str[i];
//            }
...
//            WriteConsoleOutputCharacterA(hConsole, row.data(), width, topLeft, &written);  // ANSI ¹öÀü »ç¿ë
//            topLeft.Y++;
//        }
//    }
//
...
void DrawBattleChoiceInven(ConsoleBuffer& buffer) {
    ...
    buffer.DrawAt(10, 38, L"¦­    ½ºÅ³              ¦­");
    buffer.DrawAt(10, 39, L"¦­                      ¦­");
    buffer.DrawAt(10, 40, L"¦­   ¢º ÀÎº¥Åä¸®        ¦­");
    buffer.DrawAt(10, 41, L"¦­                      ¦­");
    buffer.DrawAt(10, 42, L"¦­    µµ¸ÁÄ¡±â          ¦­");
    buffer.DrawAt(10, 43, L"¦­                      ¦­");
    buffer.DrawAt(10, 44, L"¦­                      ¦­");
...
void DrawBattleChoiceRunAway(ConsoleBuffer& buffer) {
    ...
    buffer.DrawAt(10, 40, L"¦­    ÀÎº¥Åä¸®          ¦­");
    buffer.DrawAt(10, 41, L"¦­                      ¦­");
    buffer.DrawAt(10, 42, L"¦­   ¢º µµ¸ÁÄ¡±â        ¦­");
    buffer.DrawAt(10, 43, L"¦­                      ¦­");
    buffer.DrawAt(10, 44, L"¦­                      ¦­");
    buffer.DrawAt(10, 45, L"¦±¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦°");
}
...
void DrawSlimeImage(ConsoleBuffer& buffer)
{
    ...
    buffer.DrawAt(50, 11, L"¦­                                     ¦­  ½½¶óÀÓ  ¦­ ");    
    buffer.DrawAt(50, 12, L"¦­                                     ¦­¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦°");
    buffer.DrawAt(50, 13, L"¦­                                     ¦­");
    buffer.DrawAt(50, 14, L"¦­                                     ¦­");
    buffer.DrawAt(50, 15, L"¦­                                     ¦­");
    buffer.DrawAt(50, 16, L"¦­                                     ¦­");
    buffer.DrawAt(50, 17, L"¦­                 ¡Û                  ¦­");
    buffer.DrawAt(50, 18, L"¦­             ¡Û       ¡Û             ¦­");
    buffer.DrawAt(50, 19, L"¦­         ¡Û               ¡Û         ¦­");
    buffer.DrawAt(50, 20, L"¦­       ¡Û                   ¡Û       ¦­");
    buffer.DrawAt(50, 21, L"¦­     ¡Û                       ¡Û     ¦­");
    buffer.DrawAt(50, 22, L"¦­    ¡Û       ¡Ü         ¡Ü     ¡Û    ¦­");
    buffer.DrawAt(50, 23, L"¦­    ¡Û       ¡Ü         ¡Ü     ¡Û    ¦­");
    buffer.DrawAt(50, 24, L"¦­     ¡Û                       ¡Û     ¦­");
    buffer.DrawAt(50, 25, L"¦­      ¡Û                     ¡Û      ¦­");
    buffer.DrawAt(50, 26, L"¦­        ¡Û                 ¡Û        ¦­");
    buffer.DrawAt(50, 27, L"¦­          ¡Û ¡Û ¡Û ¡Û ¡Û ¡Û          ¦­");
...
void DrawSkillList(ConsoleBuffer& buffer)
{
    ...
    buffer.DrawAt(30, 32, L"¦­        ÆÄÀÌ¾îº¼                                                   ¦­");
    buffer.DrawAt(34, 33, L"¦­                                                                   ¦­");
    buffer.DrawAt(34, 34, L"¦­        Èú                                                         ¦­");
    buffer.DrawAt(34, 35, L"¦­                                                                   ¦­");
    buffer.DrawAt(32, 36, L"¦­        ¸ÞÅ×¿À                                                     ¦­");
    buffer.DrawAt(34, 37, L"¦­                                                                   ¦­");
    buffer.DrawAt(31, 38, L"¦­        µÚ·Î°¡±â                                                   ¦­");
...
void DrawInvenList(ConsoleBuffer& buffer)
{
    buffer.DrawAt(34, 30, L"¦®¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¯");
    buffer.DrawAt(34, 31, L"¦­                                                                   ¦­");
    buffer.DrawAt(30, 32, L"¦­        Ã¼·Â¹°¾à                                                   ¦­");
    buffer.DrawAt(34, 33, L"¦­                                                                   ¦­");
    buffer.DrawAt(34, 34, L"¦­        ¸¶³ª¹°¾à                                                   ¦­");
    buffer.DrawAt(34, 35, L"¦­                                                                   ¦­");
    buffer.DrawAt(32, 36, L"¦­        ¿¤¸¯¼­                                                     ¦­");
    buffer.DrawAt(34, 37, L"¦­                                                                   ¦­");
    buffer.DrawAt(32, 38, L"¦­        µÚ·Î°¡±â                                                   ¦­");
    buffer.DrawAt(34, 39, L"¦­                                                                   ¦­");
...
void printUserInfo2(ConsoleBuffer& buffer)
{
    Manager& manager = Manager::getInstance();
    buffer.DrawAt(8, 2, L"¦®¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¯");
    buffer.DrawAt(8, 3, L"          [ÇÃ·¹ÀÌ¾î  Á¤º¸]");
    buffer.DrawAt(10, 4, L"¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬");

    wstring wstr(player->GetName().size(), L'\0');
    std::mbstowcs(&wstr[0], player->GetName().c_str(), player->GetName().size());
    buffer.DrawAt(10, 5, L"ÀÌ¸§ : " + wstr);
    buffer.DrawAt(10, 6, L"Ã¼·Â / ÃÖ´ëÃ¼·Â : " + to_wstring(player->GetHp()) + L" / " + to_wstring(player->GetMaxHp()));
    buffer.DrawAt(10, 7, L"¸¶³ª / ÃÖ´ë¸¶³ª : " + to_wstring(player->GetMp()) + L" / " + to_wstring(player->GetMaxMp()));
    buffer.DrawAt(10, 8, L"°ø°Ý·Â : " + to_wstring(player->GetAtk()));
    buffer.DrawAt(10, 9, L"¹æ¾î·Â : " + to_wstring(player->GetDef())) ;
    buffer.DrawAt(8, 10, L"¦±¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦¬¦°");
    //buffer.DrawAt(10, 8, L"Å©¸®Æ¼ÄÃ È®·ü : " + to_wstring(player->GetCriRate()) + L"%");
    //buffer.DrawAt(10, 9, L"Å©¸®Æ¼ÄÃ µ¥¹ÌÁö :" + to_wstring(player->GetCriDamage()) + L"%");
    /*buffer.DrawAt(70, 9, L"È¸ÇÇÀ² :" + to_wstring(player->GetAvoidRate()) + L"%");
    buffer.DrawAt(70, 10, L"¼Óµµ :" + to_wstring(player->GetSpeed()));*/
    //buffer.DrawAt(10, 10, L"¼ÒÁö±Ý :" + to_wstring(player->GetMoney()) + L"°ñµå");

    string mapName = manager.map.GetMapName(manager.map.GetcurrenteMap());
    wstring wstr2(mapName.size(), L'\0');
    std::mbstowcs(&wstr2[0], mapName.c_str(), mapName.size());
    //buffer.DrawAt(10, 11, L"ÇöÀç À§Ä¡ :" + wstr2);
}
...
void renderBuffer2(Map* currentMap)
{
    ...
    int startX = playerX - (ViewSizeX / 2); // ÇÃ·¹ÀÌ¾î¸¦ Áß½ÉÀ¸·Î viewSizeÀÇ Å©±â Àý¹Ý¾¿ ÁÂ¿ì ·»´õ¸µ ¹üÀ§
    int endX = playerX + (ViewSizeX / 2);
    int startY = playerY - (ViewSizeY / 2); // ÇÃ·¹ÀÌ¾î¸¦ Áß½ÉÀ¸·Î viewSizeÀÇ Å©±â Àý¹Ý¾¿ »óÇÏ ·»´õ¸µ ¹üÀ§
    ...
    //// ÄÜ¼Ö Áß¾Ó¿¡ ¸ÂÃß±â À§ÇØ ÁÂÇ¥ °è»ê
    int offsetX = (consoleWidth - mapWidth) / 2 -20;
    int offsetY = (consoleHeight - mapHeight) / 2;

    //// ÁÙ ¸ÂÃß±â À§ÇØ À§ÂÊ¿¡ ºó ÁÙ Ãâ·Â
    for (int i = 0; i < offsetY; ++i)
    {
        std::cout << std::endl;
    }

    // ½Ã¾ß ¹üÀ§ ³»¿¡¼­ Ãâ·Â (XÃà¿¡ °ø¹é Ãß°¡)
    for (int y = startY; y <= endY; ++y)
    {
        //// ¿ÞÂÊ¿¡ °ø¹é Ãß°¡ÇÏ¿© XÃà Áß¾Ó ¸ÂÃã
...
void MovePlayer(Map* currentMap)
{
    ...
    case (int)EGameObject::MONSTER:
    {
     ...
     /*   switch (rand() % 3)
        ...
        ConsoleBuffer buffer(150, 51);  // ÄÜ¼Ö Å©±â¸¦ ¼³Á¤Çß½À´Ï´Ù
            ...
            if (manager.input.GetKeyDown(eKeyCode::Space))
            {
                ...
                else if (menuSelect == 2)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                        case 0:
                            buffer.DrawAt(37, 32, L"¢º ÆÄÀÌ¾îº¼");
                            break;
                        case 1:
                            buffer.DrawAt(41, 34, L"¢º Èú");
                            break;
                        case 2:
                            buffer.DrawAt(39, 36, L"¢º ¸ÞÅ×¿À");
                            break;
                        case 3:
                            buffer.DrawAt(39, 38, L"¢º µÚ·Î°¡±â");
                            break;
                        default:
                            break;
                        }
                ...
                else if (menuSelect == 3)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                        case 0:
                            buffer.DrawAt(37, 32, L"¢º Ã¼·Â¹°¾à");
                            break;
                        case 1:
                            buffer.DrawAt(41, 34, L"¢º ¸¶³ª¹°¾à");
                            break;
                        case 2:
                            buffer.DrawAt(39, 36, L"¢º ¿¤¸¯¼­");
                            break;
                        case 3:
                            buffer.DrawAt(39, 38, L"¢º µÚ·Î°¡±â");
                            break;
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            popup.DrawOutLine(5, 5, 60, 20, L" ÇÃ·¹ÀÌ¾î Á¤º¸");
            inputAnyKey();
            system("cls");
        }
